...
RT_plt <- StoppSignal_cleanRT %>% 
  ...
  geom_point(data = StoppSignal_individuals,
             aes(color = Stim_verb,
                 color = after_scale(darken(color, .1, space = "HLS"))),
             fill = "white",
             stat = "identity",
  ...
  geom_path(data = StoppSignal_individuals, 
            aes(group = Part_nr),
            stat = "identity",
            color = "black",
            alpha = .15,
...
SRT_comb <- ggarrange(SSRT_plt, Error_plt + rremove("y.text"),
                      labels = c("SSRT", "Error"),
                      ncol = 2, nrow = 1, align = "h", widths = c(1,1), label.x = 0,
                      font.label=list(color="black",size=28))

annotate_figure(SRT_comb,
                top =text_grob( "Stop-Signal Task", face = "bold", size = 32))



save_n <- "StopSignal.png"
